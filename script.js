(function () {
  ...
  function isEmbeddedWebView() {
    ...
    return /Android/i.test(ua) && /;\s*wv\)|WebView/i.test(ua);
  }

  function blessAllAudioOnGesture() {
    userInteracted = true;
    audioUnlocked = true;
    ...
    if (audioKeeper) {
      ...
      try {
        var keeperPlay = audioKeeper.play();
        if (keeperPlay && keeperPlay.catch) keeperPlay.catch(function () {});
      } catch (err) {}
    }
  }

  function startAudioKeeper() {
    if (!audioKeeper) return Promise.resolve(false);

    audioKeeper.loop = true;
    audioKeeper.muted = true;
    audioKeeper.volume = 0;

    if (audioSessionReady && !audioKeeper.paused) {
  ...
  function startAudioPlayback(el, audioKey) {
    ...
    return startAudioKeeper().then(function () {
      return playWithTimeout(el, 4000);
    }).then(function () {
      el.muted = false;
  ...
  async function canAutoplayWithSound() {
    ...
    var el = audioMap['aud-0'];
    if (!el) return false;

    await waitForCanPlay(el);
    el.currentTime = 0;
    el.muted = false;
    applyAudioVolume(el, 'aud-0');

    try {
      await playWithTimeout(el, 2000);
      el.pause();
      el.currentTime = 0;
      return true;
    } catch (err) {
  ...
  async function shouldShowStartGate() {
    if (userInteracted) return false;
    if (isEmbeddedWebView()) return true;
    return !(await canAutoplayWithSound());
  }
  ...
  function waitForStartGate() {
    return new Promise(function (resolve) {
      ...
      function beginStory() {
        ...
        startBtn.removeEventListener('click', beginStory);
        startGate.removeEventListener('pointerdown', onGateTap);
        blessAllAudioOnGesture();
        unlockWaiters.forEach(function (r) { r(); });
        unlockWaiters = [];
        resolve();
      }

      function onGateTap(e) {
        if (e.target === startGate) beginStory();
      }

      startBtn.addEventListener('click', beginStory);
      startGate.addEventListener('pointerdown', onGateTap);
    });
  }
  ...
  async function init() {
    ...
    setupAudioUnlock();
    setupReplay();
    var needStartGate = await shouldShowStartGate();
    if (needStartGate) {
